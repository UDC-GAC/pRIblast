...
vector<DbWrapper> DbReader::LoadDatabases() {
  ...
  while (true) {
    ...
    int size = LoadChunk(start_pos, seq_length, seqs, seq_length_rep, access,
                         cond_access, names, suffix_array, start_hash,
                         end_hash);
    if (size == 0) {
      return dbs;
    }
...
int DbReader::LoadChunk(vector<int> &start_pos, vector<int> &seq_length, vector<unsigned char> &seqs, vector<int> &seq_length_rep, vector<vector<float>> &access, vector<vector<float>> &cond_access, vector<string> &names, vector<int> &suffix_array, vector<vector<int>> &start_hash, vector<vector<int>> &end_hash) {
  int ret, aux_i = 0;
